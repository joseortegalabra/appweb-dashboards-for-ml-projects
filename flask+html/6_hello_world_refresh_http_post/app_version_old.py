...
# SA de compute engine que tiene los permisos para recibir pubsub
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'cmpc-innovation-cd4ml-test-sa-compute-engine.json'
...
# --------------------------------------------   ACTUALIZAR PÁGINA DE ACUERDO A UN EVENTO HTTP POST   --------------------------------------------
### MI EJEMPLO HTTP POST PERO QUE HACE QUE SE ACTUALIZE CONSTAMENTE LA WEB
# @app.route('/trigger_http_refresh', methods=['POST'])
# def handle_post():
#     data = request.get_json()
#     msg = data['data']
#     print('mensaje string obtenido desde pubsub: ', msg)
#     return redirect(url_for('events')) # REDIRIGE Y HACE QUE SE ACTUALIZE CONSTAMENTE
...
#----------------------------------------------------------------------------
####### AGREGAR EVENT PARA QUE LA APP NO ESTÉ CONSTATEMENTE ESCUCHANDO Y ACUALIZE SIEMPRE LA WEB #######
...
#     # Desbloquear la función generadora
#     event.set()
...
#     def generate_refresh_custom():
#         while True:
#             # Esperar hasta que se reciba una solicitud POST
#             event.wait()
...
#             # Volver a bloquear la función generadora
#             event.clear()
